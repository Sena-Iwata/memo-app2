<template>
    <div class="max-w-xl mx-auto p-2 flex justify-between">
        <div class="flex items-center">
            <DocumentSvg/>
            <h2 class="font-bold text-lg text-gray-800 dark:text-gray-200">
                保存されたメモ
            </h2>
        </div>

        <div class="bg-orange-100 text-orange-800 text-sm font-semibold px-3 py-1 rounded-full dark:text-red-400 dark:bg-gray-800">
            <p>{{memos.length}}件</p>
        </div>

    </div>
    <p v-if="memos.length === 0" class="text-center text-gray-500 p-5">
        まだメモがありません。上のフォームから最初のメモを追加しましょう！
    </p>
    <div v-for="memo in memos" :key="memo.id" class="max-w-xl mx-auto bg-white rounded-xl shadow-md p-6 mt-4 flex flex-col group dark:bg-gray-800 rounded-xl shadow-md p-4 mt-4 flex flex-col gap-2 group">
        <div class="flex justify-between">
            <p class="dark:text-gray-200">{{ memo.content }}</p>
            <div class="flex items-center space-x-2">
                <button @click="$emit('edit-memo', memo)" class="p-1 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200>"><EditSvg/></button>
                <button @click="$emit('delete-memo',memo.id)" class="p-1 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200>"><TrashSvg/></button>
            </div>
        </div>
        <div class="flex">
        <span class="text-sm text-gray-400 dark:text-gray-500">{{ memo.created_at }}</span>
        </div>
    </div>

</template>
<script setup lang="ts">
import EditSvg from "@/components/svgs/EditSvg.vue";
import { PropType } from 'vue';
import DocumentSvg from "@/components/svgs/DocumentSvg.vue";
import TrashSvg from "@/components/svgs/TrashSvg.vue";
interface Memo {
    id: number;
    content: string;
    created_at: string;
    updated_at: string;
}
defineProps({
    memos:{
    type:Array as PropType<Memo[]>,
    required:true,
    },
})

defineEmits<{
    (e: 'edit-memo', memo: Memo): void;
    (e: 'delete-memo', id: number): void;
}>();
</script>
